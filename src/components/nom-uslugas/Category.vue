<template>
    <div class="row align-items-center mb-4">
        <div class="col-12 col-lg-6">
            <h4 class="py-3 m-0">{{ category.name ?? '-' }}</h4>
        </div>
    ...
    <div class="card folder-list">
        <div class="card-body">
            <Loader v-if="views.loading"/>

            <div v-if="category.children && category.children.length" class="row">
                <div v-for="childCategory in category.children" class="col-12 col-lg-3">
                    <RouterLink :to="{name: 'NomUslugaCategory', params: {id: childCategory.id}}" class="folder-list-item">
            ...
            <div v-if="category.uslugas && category.uslugas.length" class="row mt-4">
                <div v-for="usluga in category.uslugas" class="col-12 col-lg-3">
                    <a href="#">
                        {{usluga.name}}
                    </a>
                </div>
...
export default {
    ...
    methods: {
        loadCategory() {
            axios.get(`${import.meta.env.VITE_API_FF_SERVER}/api/nom-usluga-category/${this.$route.params.id}`)
                .then(response => {
